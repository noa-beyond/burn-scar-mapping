...
def find_cloud_coverage (bbox,start_date, end_date, level = 'sentinel-2a', cloud_coverage_threshold=10):
    # Search for products
    results = search_products(BBox(bbox=bbox, crs=CRS.WGS84), (start_date, end_date))
    ...
    products = products[products.cloud_coverage <= cloud_coverage_threshold] # Apply threshold
...
def search_sentinel(start_date, end_date, aoi_polygon ,bbox, data_collection = "SENTINEL-2", level = 'L2A'):
    ...
    products_sorted  = products.sort_values('date', ascending=True).reset_index() # Sort by date
